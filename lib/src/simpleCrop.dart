...
class ImgCrop extends StatefulWidget {
  ...
  final double? chipHeight; // 裁剪半径
  final double? chipWidth; // 裁剪半径
  final double chipRadius; // 裁剪半径
  final double chipHeightRatio; // 裁剪半径
...
class ImgCropState extends State<ImgCrop> with TickerProviderStateMixin {
  ...
  Future<File> cropCompleted(File file, {int pictureQuality = 900}) async {
  ...
  Size get _boundaries {
    return _surfaceKey.currentContext!.size! -
        Offset(_kCropHandleSize, _kCropHandleSize) as Size;
  }
  ...
  Rect _calculateDefaultArea({
    ...
    if(widget.chipWidth != null) {
      _areaOffset = (_deviceWidth - (widget.chipWidth! * 2));
    } else {
      _areaOffset = (_deviceWidth - (widget.chipRadius * 2));
    }

    final _areaOffsetRadio = _areaOffset / _deviceWidth;
    width = 1.0 - _areaOffsetRadio;

    if(widget.chipHeight != null) {
      height = widget.chipHeight! * width / widget.chipWidth!;
    } else {
      height =
